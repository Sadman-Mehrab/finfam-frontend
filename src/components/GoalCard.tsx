...
const GoalCard = ({
  ...
  const handleDelete = async () => {
    try {
      ...
      await axios.delete(`${config.BACKEND_URL}/api/goals/${goal._id}`, {
        headers: {
  ...
  return (
    <div className="card w-96 bg-base-100 shadow-xl transform transition hover:-translate-y-1">
      <div className="card-body">
        ...
        <progress
          className={`progress  ${
            goalProgress.totalAmount <= goalProgress.totalCompleted
              ? "progress-success"
              : "progress-info"
          } w-56`}
        ...
        <div className="flex flex-row justify-between">
          <button onClick={handleDelete} className="btn btn-circle btn-secondary">
            <svg
